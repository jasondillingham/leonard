...
2. **Tree-sitter binding choice** — ~~`smacker/go-tree-sitter` is most popular but has CGo. Pure-Go alternatives exist but are less complete per-language. Decide before phase 1 starts.~~ **Resolved (phase 1):** the Go extractor uses the standard library's `go/parser` + `go/ast` instead of a tree-sitter binding. Phase 1 is Go-only and the stdlib is pure-Go (no CGo build pain), faster than tree-sitter for Go specifically, and handles generics, type aliases, and method receivers without extra grammar work. ~~Tree-sitter remains the right call for phase 2 (Python, TypeScript) where the stdlib does not help — binding choice (CGo `smacker/go-tree-sitter` vs a pure-Go WASM-based alternative) is deferred until that work starts.~~ **Resolved (phase 2, Python):** the Python extractor uses `github.com/go-python/gpython`'s `parser` + `ast` packages. Same shape as the phase-1 Go decision — gpython is a pure-Go Python 3 parser (no CGo, fitting the modernc.org/sqlite philosophy this project already commits to), maintained, with a real AST that handles indentation, multi-line strings, decorators, and varargs correctly without re-implementing Python grammar. Tradeoffs: gpython tracks Python 3.4 syntax, so newer constructs (f-strings, walrus `:=`, `match`, PEP 695 generics, annotated assignments `x: int = 1`) will fail to parse — acceptable for v0 since the indexer logs and moves on per file. If newer Python becomes a problem in practice, the upgrade path is WASM tree-sitter via `wazero`. **Resolved (phase 3, TypeScript):** the TypeScript extractor uses a hand-rolled scanner in `internal/parse/typescript.go` — strip strings/comments/template literals, lex, then walk a small state machine driven by the modifier-and-keyword prefix of each top-level statement. Same pure-Go-no-CGo philosophy as phases 1 and 2, but the candidate library landscape differed: `smacker/go-tree-sitter` is the most mature TS grammar but requires CGo (Windows build pain, static-binary friction); WASM tree-sitter via `wazero` is pure-Go but bloats the binary by 5–10 MB just to load a single grammar; the v0 brief is intentionally narrow (top-level functions, classes + their methods, interfaces, type aliases, `const`/`let`/`var`) and the scanner is ~600 lines including comments and the test suite stays at parity with the Python lane. Tradeoffs documented inline: object-type-literal return types (`function f(): { x: T } { … }`) are mis-extracted — the first `{` after the param list is treated as the function body, so EndLine is wrong on those specific declarations. Other edge cases left for v0: decorators, default exports, JSX element extraction, generic type parameters (we record only the simple name), regex literals at top level (not stripped — top-level regex is rare enough that brace-counting holds). Upgrade path if those become problems in practice is the same as Python — WASM tree-sitter via `wazero`. Recorded via `leonard record_decision` once that MCP tool ships.
